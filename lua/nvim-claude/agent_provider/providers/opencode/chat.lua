-- OpenCode provider: chat transport via tmux
...
local function ensure_pane()
  ...
  if not pane then
    pane = tmux.create_pane(cfg.spawn_command or 'opencode')
  end
  return pane
end
...
local function spawn_targeted_pane(base_pane, initial_text)
  ...
  local spawn_cmd = cfg.targeted_spawn_command or cfg.spawn_command or 'opencode'
