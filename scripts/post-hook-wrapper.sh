...
if [ -n "$FILE_PATH" ]; then
    ...
    echo "[$(date)] Calling nvim-rpc with FILE_PATH_B64: $FILE_PATH_B64" >> "$LOG_FILE"
    RESULT=$(TARGET_FILE="$FILE_PATH" "$SCRIPT_DIR/nvim-rpc.sh" --remote-expr "luaeval(\"require('nvim-claude.hooks').post_tool_use_hook_b64('$FILE_PATH_B64')\")" 2>&1)
    echo "[$(date)] nvim-rpc result: $RESULT" >> "$LOG_FILE"
else
    "$SCRIPT_DIR/nvim-rpc.sh" --remote-expr 'luaeval("require(\"nvim-claude.hooks\").post_tool_use_hook_b64()")'
fi